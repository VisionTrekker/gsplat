...
class Parser:
    ...
    def __init__(
        ...
        for k in imdata:    # 遍历所有COLMAP图像 外参
            im = imdata[k]
            rot = im.R()
        ...
        for point_id, data in manager.point3D_id_to_images.items(): # 遍历每个3D点，获取观测到该3D点的 图像数据   # manager.point3D_id_to_images：3D点ID -> List存储着：看到该3D点的图像IDs 及 该3D点在对应图像的2D点索引
            for image_id, _ in data:    # 遍历每个观测到该3D点的图像，将该3D点在点云中的索引添加到 对应图像的列表中
        ...
        self.Ks_dict = Ks_dict          # 所有COLMAP相机 经图像下采样后的 内参矩阵                           Dict of camera_id -> K
        self.params_dict = params_dict  # 所有COLMAP相机的 畸变参数                                        Dict of camera_id -> params
        self.imsize_dict = imsize_dict  # 所有COLMAP相机 经图像下采样后的 图像宽、高                          Dict of camera_id -> (width, height)
        self.mask_dict = mask_dict      # 所有COLMAP相机的 mask，默认为 None                               Dict of camera_id -> mask
        ...
        for camera_id in self.params_dict.keys():   # 遍历每个COLMAP相机的 畸变参数
            ...
            self.Ks_dict[camera_id] = K_undist  # 所有COLMAP相机去畸变后的内参矩阵
            self.roi_undist_dict[camera_id] = roi_undist
            self.imsize_dict[camera_id] = (roi_undist[2], roi_undist[3])    # 所有COLMAP相机去畸变后的 图像宽、高
            self.mask_dict[camera_id] = mask    # 所有COLMAP相机去畸变后的 mask，默认为None
...
class Dataset:
    ...
    def __init__(
        ...
        self.load_depths = load_depths  # 是否使用 depth_loss，即是否要加载 深度图
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        params = self.parser.params_dict[camera_id] # 当前图像对应相机id的畸变参数
